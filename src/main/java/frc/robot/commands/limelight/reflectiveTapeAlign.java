...
public class reflectiveTapeAlign extends CommandBase {
   ...
   private final subsistemaSwerve swerve;
    ...
    public reflectiveTapeAlign(subsistemaSwerve subsistemaSwerve, LimeLightObject limelight, boolean alingToAprilTag){

        this.swerve = subsistemaSwerve;
     ...
     public void execute() {
        ...
        if(limelight.getObjetivoVisto()){
            ...
            velAvance = drivePID.calculate(limelight.getALimelight(), 0.08);
            velStrafe = strafePID.calculate(limelight.getXLimelight(), 1.5);
            velGiro = giroPID.calculate(swerve.getHeading(), 0); 
